...
namespace gv {
	...
	protected:
		gVector(Scalar* data_ptr, size_t size) :_data(data_ptr), _size(size), _row(size), _col(1) {}
		gVector(Scalar* data_ptr, size_t row, size_t col) :_data(data_ptr), _size(row* col), _row(row), _col(col) {}

	protected:
		auto& _Get_data(void) { return _data; }
		auto& _Get_size(void) { return _size; }
		auto& _Get_rols(void) { return _row; }
		auto& _Get_cols(void) { return _col; }
	public:
		...
		void move(Scalar* data_ptr, size_t size) { _data = data_ptr; _size = size; _row = size; _col = 1; }
		...
		gVector(void) :_size(0), _row(0), _col(0), _data(nullptr) {}
	...
	public:
		...
		void resize(size_t row, size_t col) { build(row * col); }
	...
	public:
		...
		const gVector& operator+=(const expr_t& expr) {
			...
			size_t expr_row = expr.rows();
			size_t expr_col = expr.cols();
			if (expr_dim != _size) {
				throw std::string("unmatched vector size !");
			}
			if (expr_row != _row) {
				throw std::string("unmatched row !");
			}
			if (expr_col != _col) {
				throw std::string("unmatched col !");
			}
	...
	void gVector<Scalar>::set(const Scalar* host_ptr) {
		cudaMemcpyToSymbol(data(), host_ptr, sizeof(Scalar)*size(), cudaMemcpyHostToDevice);
		cuda_error_check;
	}
